  ...
  return (
    <>
      <header className="flex flex-wrap items-center justify-between gap-4">
        ...
        <button
          className="h-12 rounded-lg border border-slate-200 bg-white px-4 text-sm text-slate-700 transition hover:border-slate-300 hover:bg-slate-50"
          onClick={() => setCart([])}
        >
          Reset keranjang
        </button>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
            <div>
              ...
              <p className="text-xs text-slate-500">
                Scan barcode atau pilih dari daftar.
              </p>
            </div>
            <button
              onClick={loadProducts}
              className="h-11 rounded-lg border border-slate-200 bg-white px-4 text-sm text-slate-700 transition hover:border-slate-300 hover:bg-slate-50"
            >
              Refresh
            </button>
          </div>
          <form onSubmit={handleBarcode} className="flex flex-wrap items-end gap-3 mb-4">
            <div className="flex-1 space-y-1">
              ...
              <input
                ref={barcodeInputRef}
                className="w-full h-12 rounded-lg border border-slate-300 bg-white px-4 text-sm focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-100"
            ...
            <button
              type="submit"
              className="h-12 rounded-lg bg-emerald-500 px-5 text-sm font-semibold text-white shadow-sm transition hover:bg-emerald-600"
            >
              Tambah
            </button>
          </form>
            ...
            {filteredProducts.length === 0 ? (
              ...
              filteredProducts.map((product) => (
                <button
                  ...
                  className="group flex items-center justify-between rounded-xl border border-slate-200 bg-white p-4 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-emerald-400 hover:shadow-md disabled:cursor-not-allowed disabled:opacity-60"
                >
                  <div>
                  ...
                  >
                    {product.qty > 0 ? `Stok: ${product.qty}` : "Stok habis"}
                  </span>
                </button>
              ))
            )}
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              cart.map((item) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      className="h-8 w-8 rounded-md border border-slate-300 text-slate-700 transition hover:bg-white"
                    ...
                    <button
                      ...
                      className="h-8 w-8 rounded-md border border-slate-300 text-slate-700 transition hover:bg-white disabled:cursor-not-allowed disabled:opacity-50"
                      onClick={() => updateQty(item.id, 1)}
                    >
                      +
                    </button>
          ...
          <div className="mt-6 rounded-xl border border-slate-200 bg-white p-4 shadow-sm space-y-3">
            ...
            <button
              ...
              className="w-full h-12 rounded-lg bg-emerald-500 text-sm font-semibold text-white shadow-sm transition hover:bg-emerald-600 disabled:opacity-50"
            >
          ...
          <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-lg">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-xs text-slate-500">
                  Konfirmasi jumlah uang diterima.
                </p>
              </div>
            </div>
            <div className="space-y-4">
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 h-12 rounded-lg border border-slate-300 bg-white text-sm text-slate-700 transition hover:border-slate-400"
                >
                  Batal
                </button>
                <button
                  ...
                  className="flex-1 h-12 rounded-lg bg-emerald-500 text-sm font-semibold text-white shadow-sm transition hover:bg-emerald-600 disabled:opacity-50"
                >
